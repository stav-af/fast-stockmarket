    ...
    pub fn compress(&mut self) {
        ...
        for i in 0..(len - 1) {
            ...
            if !current_hist.is_empty() && current_hist.len() > granularity_max_measurements(current_hist[0].granularity) {
                next_hist.push(Self::downgrade_granularity(current_hist, current_hist[0].granularity));
            }
        }
    }

    fn downgrade_granularity(measurements:&mut Vec<ObStat>, granularity: GRANULARITY) -> ObStat {
